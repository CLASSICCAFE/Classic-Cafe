...
function corsHeaders(){
  ...
  return {
    ...
    "access-control-allow-headers":
      "content-type,x-admin-key,x-delivery-key,x-delivery-session"
  };
...
async function ensureDeliveryTables(env){
  ...
  /*
    Existing Cloudflare DELIVERY_KEY becomes DB001
    only when no matching delivery boy exists.
  */
...
/* =====================================================
   ...
   DELIVERY:
     x-delivery-session  <-- NEW PRIMARY LOGIN
     x-delivery-key      <-- OLD BACKWARD COMPATIBILITY
===================================================== */
...
async function authorized(request,env,type){
  ...
  if(type === "delivery"){
    ...
    /*
      New OTP/session authentication.
    */

    const session =
      request.headers.get("x-delivery-session");
      ...
      try{
        ...
        const row =
          await env.DB.prepare(`
            ...
            JOIN delivery_boys b
            ON b.id=s.delivery_boy_id
            WHERE s.token=?
            AND s.expires_at>?
            AND b.active=1
            LIMIT 1
    ...
    /*
      Old Delivery Key remains supported so existing
      admin/legacy pages do not suddenly break.
    */
...
async function api(request,env,url){
  ...
  if(
    ...
    if(!boy){
      ...
      return json(
        {
          error:
            "This mobile number is not registered as Delivery Boy."
        },
        404
      );
    ...
    if(!Number(boy.active)){
      ...
      return json(
        {
          error:
            "This Delivery Boy is disabled by Admin."
        },
        403
      );
    ...
    /*
      ...
      IMPORTANT:
      This project currently uses Admin approval.
      An SMS provider is NOT connected here, so the OTP is stored
      in D1 and returned only as part of the server-side request flow.
      Admin can see the pending OTP through the admin endpoint.
    */
    ...
    return json({
      ok:true,
      message:
        "OTP request created. Admin verification required.",
      request_id:id,
  ...
  if(
    ...
    if(Number(row.used)){
      ...
      return json(
        {
          error:
            "This OTP request has already been used."
        },
        400
      );
    ...
    return json({
      ok:true,
      message:
        "Delivery Boy OTP approved successfully.",
      request_id:row.id,
  ...
  if(
    ...
    if(!Number(boy.active)){
      ...
      return json(
        {
          error:
            "Your Delivery Boy account is disabled."
        },
        403
      );
    ...
    /*
      Remove old sessions for this delivery boy.
      One active login session is enough for this dashboard.
    */
  ...
  if(
    ...
    if(
      settings.auto_assign_distance===undefined
    ){
  ...
  if(
    ...
    if(
      ...
      if(!ps.includes(body.payment_status)){
